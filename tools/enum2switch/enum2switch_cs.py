...
# (?<= PRECESING_MATCH)
EnumTypename = "A"
ListEnumTypename = re.findall(r'(?<=enum).*(?={)',
						str(InputString),
						re.I|re.M|re.DOTALL)
						
EnumTypename = ListEnumTypename[0]
EnumTypename = EnumTypename.rstrip()                # endl weg
EnumTypename = re.sub(r'\s',"", EnumTypename)		# alle Leerzeichen weg 	
print("AAAA")
print(EnumTypename)
