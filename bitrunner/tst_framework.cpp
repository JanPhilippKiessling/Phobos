...
private slots:
    void TC1();
    void TC_Bitrunner();
    void TC_Bitrunner_WholeMem_HappyPath();
    void TC_Bitrunner_SafeAndRestoreMemory();
};

void TST_Framework::TC1()
{
    MyModule1 UUT;

    QVERIFY(UUT.MyFunc1() == 1);
}
...
void vWriteByte(uint16_t const _u16ByteNr, uint8_t const _u8Data)
{
    ...
    {
        a8MemToRun[mgb8_ByteNrToManipulate] = mgu8_ManipulationValue; //bitrunner needs to find that error
...
void TST_Framework::TC_Bitrunner()
{
    ...
    ts_BIR_ClassStruct sMyBitrunner;

    // false init
    QVERIFY(u8_BIR_InitBitrunner(&sMyBitrunner, 0, MEMSIZE, _fpReadByte, _fpWriteByte) == 1);


    // correct init
    QVERIFY(u8_BIR_InitBitrunner(&sMyBitrunner, _pu8WorkMem, MEMSIZE, _fpReadByte, _fpWriteByte) == 0);


    v_BIR_PrepareMem(&sMyBitrunner);
    ...
    {
        u32StepCnt++;        
        QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //8
    }

    QVERIFY(a8MemToRun[0] == 0x80 );

    u32StepCnt++;    
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //9
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 1 );

    u32StepCnt++;

    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //10
    QVERIFY(a8MemToRun[0] == 0 );
    ...
    {
        u32StepCnt++;
        QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //16
    }
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 0x80 );

    u32StepCnt++;
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //17
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 0 );
    ...
    {
        u32StepCnt++;
        QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //24
    }
    ...
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 1);     //25
    ...
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //26
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 0 );
    ...
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 1);     //27
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 1 );

    vManipulateMemory(1, 0x00);           // byte 1 rep
    u32StepCnt++;
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //28
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 0 );

    vManipulateMemory(2, 0x01);           // byte 2 destroy
    u32StepCnt++;
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 1);     //29
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 0 );
    QVERIFY(a8MemToRun[2] == 1 );
    ...
    u32StepCnt++;
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);     //30
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 0 );
    QVERIFY(a8MemToRun[2] == 0 );
    ...
    u32StepCnt++;
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 1);     //31
    QVERIFY(a8MemToRun[0] == 0 );
    QVERIFY(a8MemToRun[1] == 0 );
    QVERIFY(a8MemToRun[2] == 0 );
    QVERIFY(a8MemToRun[3] == 1);
    ...
    // restart at step 1
    v_BIR_SetNextStepNumber(&sMyBitrunner, 1);
    QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);       //1
    QVERIFY(a8MemToRun[0] == 1);
}

/* a clean run over the whole memory */
void TST_Framework::TC_Bitrunner_WholeMem_HappyPath()
{
    uint8_t _pu8WorkMem[MEMSIZE];
    u32StepCnt = 0;
    tfp_BR_ReadByte _fpReadByte   = &u8ReadByte;
    ...
    {
        QVERIFY(u8_BIR_Step(&sMyBitrunner) == 0);
        vPrintMem(5);
    }
}


void TST_Framework::TC_Bitrunner_SafeAndRestoreMemory()
{
    tfp_BR_ReadByte _fpReadByte   = &u8ReadByte;
    tfp_BR_WriteByte _fpWriteByte = &vWriteByte;
    uint8_t _pu8WorkMem[MEMSIZE];
